...
export default function QueueStatisticsSkeleton({ includeFilter = true }) {
  ...
  if (!includeFilter) return <div aria-busy="true" aria-live="polite">{content}</div>;

  return (
    <main className="max-w-[1440px] p-4" aria-busy="true" aria-live="polite">
      ...
      <Card className="mb-4">
        <Card.Content className="grid gap-x-3 gap-y-1 p-4 lg:grid-cols-[minmax(0,26rem)_minmax(0,14rem)_auto] lg:items-end">
          ...
          <Skeleton className="h-10 w-16 rounded lg:col-start-3 lg:row-start-1" />
        </Card.Content>
      </Card>
      {content}
    </main>
